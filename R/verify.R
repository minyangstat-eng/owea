...
#'   would exceed it, you are asked whether to proceed (interactive session) or
#'   stopped with a message to coarsen \code{step} or raise \code{max_points}.
...
verify_optimality <- function(support, weights = NULL,
  ...
  if (use_set) {
    ...
    # as optimal_design's check_global does).
    by   <- .expand_stage_step(as.numeric(step), is_factor, sum(!is_factor),
                               length(design_box))
    npts <- prod(ifelse(is_factor, nlevels, round((hi - lo) / by) + 1))
    if (npts > max_points) {
      msg <- sprintf(paste0("verify_optimality(): the design_box + step grid has %.0f design ",
                            "points (> max_points = %.0f). Increase 'step' (a coarser grid) to ",
                            "reduce the number of design points, or raise 'max_points'."),
                     npts, max_points)
      proceed <- interactive() &&
        isTRUE(utils::askYesNo(paste(msg, "Build all of them anyway?"), default = FALSE))
      if (!proceed) stop(msg, call. = FALSE)
      warning(paste(msg, "Proceeding anyway."), call. = FALSE)
    }
    X <- candidate_grid(design_box, step)
  }
  storage.mode(X) <- "double"

  # ---- infer k and theta_use (mirrors optimal_design) --------------------
  probe <- as.numeric(X[1, ])
  ...
  dmax <- vapply(seq_len(nrow(support)),
  ...
  scaled <- .scale_info(.build_info_data(X, info_mode, info_vector, info_matrix,
  ...
  ve <- verify_equiv_cpp(as.integer(p), as.matrix(wb_use), info_mode, scaled,
                         oi, as.matrix(infor0))
  ...
  list(is_optimal      = list(
         value = isTRUE(ve$max_d <= tol),
         note  = sprintf(paste0("'value' is TRUE when max_sensitivity <= tol; ",
                                "tol = %g was used."), tol)),
       max_sensitivity = ve$max_d,
       criterion       = crit,
       information     = M,
       maximiser       = X[ve$index, ],
